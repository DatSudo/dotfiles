...
def pretty_print_group(key, aliases, highlight=None):
    if len(aliases) == 0:
        return
    group_hl_formatter = lambda g, hl: alias_termcolor.colored(hl, 'yellow').join([alias_termcolor.colored(part, 'red') for part in ('[%s]' % g).split(hl)])
    alias_hl_formatter = lambda alias, hl: alias_termcolor.colored(hl, 'yellow').join([alias_termcolor.colored(part, 'green') for part in ('\t%s = %s' % alias[0:2]).split(hl)])
    group_formatter = lambda g: alias_termcolor.colored('[%s]' % g, 'red')
    alias_formatter = lambda alias: alias_termcolor.colored('\t%s = %s' % alias[0:2], 'green')
